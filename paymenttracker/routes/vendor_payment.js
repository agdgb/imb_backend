...
router.get('/', async (req, res) => {
  try {
    const result = await VendorPayment.find();
    res.json(result);
  } catch (error) {
...
router.get('/:payment_status', async (req, res) => {
  try {
    const result = await VendorPayment.find({payment_status:req.params.payment_status});
    res.json(result);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
router.get('/:vendor_id', async (req, res) => {
  try {
    const result = await VendorPayment.find({vendor_id:vendor_id});
    res.json(result);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
router.put('/:transaction_id', async (req, res) => {
  try {
    ...
    if(transaction.length > 0) {
        const editedVendorPayment = new VendorPayment(req.body);
        transaction[0].balance_amount = transaction[0].balance_amount - editedVendorPayment.amount;
...
// const express = require('express')
// const router = express.Router()
// const VenderPayment = require('../models/vendorPayment');
// const vendorPayment = require('../models/vendorPayment');
// // const vendorPayment = require('../models/vendorPayment');
// // const vendorPayment = require('../models/vendorPayment');
...
// router.post('/', async(req, res) => {
//     try {
//         const vendorPayment = new VenderPayment(req.body)
